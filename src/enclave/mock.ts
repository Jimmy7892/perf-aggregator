...
export class MockEnclaveService extends BaseEnclaveService {
  ...
  async submitKey(sessionId: string, payload: EncryptedPayload): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      console.log(`✅ Credentials stored for session ${sessionId}, exchange: ${credentials.exchange}`);
      return { success: true };
  ...
  async requestAggregates(sessionId: string): Promise<{
    ...
    console.log(`📊 Computing aggregates for session ${sessionId} (DEV MOCK)`);
    ...
    console.log(`✅ Aggregates computed and signed for session ${sessionId}`);
